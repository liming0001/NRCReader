...
@property (nonatomic, strong) CustomerInfo *curInfo;

@property (nonatomic, strong) AlertbuttonClickAction alertHarder;

@end

@implementation EPPayKillInfoView
...
-(instancetype)initWithNRCustomerInfo:(CustomerInfo *)customInfo{
    self = [self init];
    if (self) {
...
- (void) _initializeUI {
    ...
    self.guestNumberLab.text = [NSString stringWithFormat:@"客人洗码号:%@",self.curInfo.washNumberValue];
    ...
    self.payTypeLab = [UILabel new];
    self.payTypeLab.textColor = [UIColor redColor];
    self.payTypeLab.font = [UIFont systemFontOfSize:16];
    self.payTypeLab.numberOfLines = 0;
    self.payTypeLab.text = [NSString stringWithFormat:@"下注类型:%@",self.curInfo.resultString];
    self.payTypeLab.textAlignment = NSTextAlignmentCenter;
    [self.moneyInfoView addSubview:self.payTypeLab];
    ...
    self.resultLab.text = [NSString stringWithFormat:@"开牌结果:%@",self.curInfo.kaiPaiResult];
    self.resultLab.textAlignment = NSTextAlignmentCenter;
    [self.moneyInfoView addSubview:self.resultLab];
    [self.resultLab mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.payTypeLab.mas_bottom).offset(20);
        make.left.equalTo(self.moneyInfoView).offset(10);
        make.height.mas_offset(20);
        make.centerX.equalTo(self.moneyInfoView);
    }];
    ...
    self.totalMoneyLab.text = [NSString stringWithFormat:@"赔注金额:%.f",self.curInfo.resultValue];
...
+ (EPPayKillInfoView *)showInWindowWithNRCustomerInfo:(CustomerInfo *)customerInfo handler:(AlertbuttonClickAction)handler{
    EPPayKillInfoView *popUpView = [[EPPayKillInfoView alloc] initWithNRCustomerInfo:customerInfo];
