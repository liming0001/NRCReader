...
@property (nonatomic, strong) tigerresultAlertbuttonClickAction alertHarder;
@property (nonatomic, assign) int isAllOrHalf;

@end

@implementation EPTigerShowView
...
+ (void)showInWindowWithNRChipResultInfo:(NRChipResultInfo *)resultInfo handler:(tigerresultAlertbuttonClickAction)handler{
    ...
    EPTigerShowView *popUpView = [[EPTigerShowView alloc] initWithNRChipResultInfo:resultInfo];
...
- (void)secondButtonAction:(UIButton *)btn{
    btn.selected = !btn.selected;
    if (btn.selected) {
        self.isAllOrHalf = 2;
