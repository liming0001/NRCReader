...
NSString *const AsyncUdpSocketExceptionN = @"AsyncUdpSocketExceptionN";
NSString *const AsyncUdpSocketErrorDomainN = @"AsyncUdpSocketErrorDomainN";

#if MAC_OS_X_VERSION_MIN_REQUIRED < MAC_OS_X_VERSION_10_5
// Mutex lock used by all instances of AsyncUdpSocketN, to protect getaddrinfo.
...
@interface AsyncUdpSocketN (Private)
...
/**
 * The AsyncSendPacketN encompasses the instructions for a single send/write.
**/
@interface AsyncSendPacketN : NSObject
{
@public
    NSData *buffer;
    NSData *address;
...
@implementation AsyncSendPacketN
...
/**
 * The AsyncReceivePacketN encompasses the instructions for a single receive/read.
**/
@interface AsyncReceivePacketN : NSObject
{
@public
    NSTimeInterval timeout;
    long tag;
...
@implementation AsyncReceivePacketN
...
@implementation AsyncUdpSocketN
...
{
    BOOL bangd = [self bindToAddress:nil port:port error:errPtr];
    NSLog(@"bangd = %d",bangd);
    return bangd;
}
...
- (BOOL)bindToAddress:(NSString *)host port:(UInt16)port error:(NSError **)errPtr
{
    ...
    {
        [NSException raise:AsyncUdpSocketExceptionN
...
- (BOOL)connectToAddress:(NSData *)remoteAddr error:(NSError **)errPtr
{
    ...
    if(errPtr)
    {
        ...
        *errPtr = [NSError errorWithDomain:AsyncUdpSocketErrorDomainN
...
- (NSError *)getSocketError
{
    ...
    return [NSError errorWithDomain:AsyncUdpSocketErrorDomainN code:AsyncUdpSocketCFSocketError userInfo:info];
}
...
- (NSError *)getIPv4UnavailableError
{
    ...
    return [NSError errorWithDomain:AsyncUdpSocketErrorDomainN code:AsyncUdpSocketIPv4Unavailable userInfo:info];
}
...
- (NSError *)getIPv6UnavailableError
{
    ...
    return [NSError errorWithDomain:AsyncUdpSocketErrorDomainN code:AsyncUdpSocketIPv6Unavailable userInfo:info];
}
...
- (NSError *)getSendTimeoutError
{
    ...
    return [NSError errorWithDomain:AsyncUdpSocketErrorDomainN code:AsyncUdpSocketSendTimeoutError userInfo:info];
}
- (NSError *)getReceiveTimeoutError
{
    ...
    return [NSError errorWithDomain:AsyncUdpSocketErrorDomainN code:AsyncUdpSocketReceiveTimeoutError userInfo:info];
}
...
- (BOOL)sendData:(NSData *)data withTimeout:(NSTimeInterval)timeout tag:(long)tag
{
    ...
    AsyncSendPacketN *packet = [[AsyncSendPacketN alloc] initWithData:data address:nil timeout:timeout tag:tag];
...
- (BOOL)sendData:(NSData *)data toHost:(NSString *)host port:(UInt16)port withTimeout:(NSTimeInterval)timeout tag:(long)tag
{
    ...
    AsyncSendPacketN *packet = nil;
    
    if(address4 && theSocket4)
        packet = [[AsyncSendPacketN alloc] initWithData:data address:address4 timeout:timeout tag:tag];
    else if(address6 && theSocket6)
        packet = [[AsyncSendPacketN alloc] initWithData:data address:address6 timeout:timeout tag:tag];
    else
        return NO;
...
- (BOOL)sendData:(NSData *)data toAddress:(NSData *)remoteAddr withTimeout:(NSTimeInterval)timeout tag:(long)tag
{
    ...
    AsyncSendPacketN *packet = [[AsyncSendPacketN alloc] initWithData:data address:remoteAddr timeout:timeout tag:tag];
...
- (CFSocketRef)socketForPacket:(AsyncSendPacketN *)packet
{
    if(!theSocket4)
        return theSocket6;
    if(!theSocket6)
...
- (void)receiveWithTimeout:(NSTimeInterval)timeout tag:(long)tag
{
    ...
    AsyncReceivePacketN *packet = [[AsyncReceivePacketN alloc] initWithTimeout:timeout tag:tag];
                  ...
                  withData:(const void *)pData
{
    ...
    switch (type)
    {
        ...
        case kCFSocketWriteCallBack:
            ...
            break;
        default:
//            NSLog (@"AsyncUdpSocketN %p received unexpected CFSocketCallBackType %d.", self, type);
            break;
    }
}
...
static void MyCFSocketCallback(CFSocketRef sref, CFSocketCallBackType type, CFDataRef address, const void *pData, void *pInfo)
{
    ...
    AsyncUdpSocketN *theSocket = [[(AsyncUdpSocketN *)pInfo retain] autorelease];
