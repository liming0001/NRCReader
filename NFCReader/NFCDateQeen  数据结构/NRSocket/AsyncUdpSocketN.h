...
@class AsyncSendPacketN;
@class AsyncReceivePacketN;

extern NSString *const AsyncUdpSocketExceptionN;
extern NSString *const AsyncUdpSocketErrorDomainN;

enum AsyncUdpSocketErrorN
{
...
typedef enum AsyncUdpSocketErrorN AsyncUdpSocketErrorN;

@interface AsyncUdpSocketN : NSObject
{
	...
	AsyncSendPacketN *theCurrentSend;
	NSTimer *theSendTimer;
	
	NSMutableArray *theReceiveQueue;
	AsyncReceivePacketN *theCurrentReceive;
...
/**
 * Creates new instances of AsyncUdpSocketN.
**/
- (id)init;
...
/**
 * Creates new instances of AsyncUdpSocketN that support only IPv4 or IPv6.
...
/**
 * When you create an AsyncUdpSocketN, it is added to the runloop of the current thread.
...
/**
 * Returns the current run loop modes the AsyncSocketN instance is operating in.
 * The default set of run loop modes is NSDefaultRunLoopMode.
**/
...
@interface NSObject (AsyncUdpSocketDelegateN)

/**
 * Called when the datagram with the given tag has been sent.
**/
- (void)onUdpSocket:(AsyncUdpSocketN *)sock didSendDataWithTag:(long)tag;

/**
 * Called if an error occurs while trying to send a datagram.
 * This could be due to a timeout, or something more serious such as the data being too large to fit in a sigle packet.
**/
- (void)onUdpSocket:(AsyncUdpSocketN *)sock didNotSendDataWithTag:(long)tag dueToError:(NSError *)error;
 ...
 * and AsyncUdpSocketN will continue as if the packet never arrived.
...
**/
- (void)onUdpSocketDidClose:(AsyncUdpSocketN *)sock;
