...
@interface ThreeFairsManual ()<UICollectionViewDelegate,UICollectionViewDataSource>
...
@implementation ThreeFairsManual
...
- (void)CalcuteCowWithPointForIndex:(NSInteger)indexRow{
    ...
    fairsPointShowView.fairsPointsResultBlock = ^(int curPoint) {
        ...
        if (curPoint==99) {
            curCustomer.cowPointValue = @"没点";
        }else if (curPoint==10){
            curCustomer.cowPointValue = @"小三公";
        }else if (curPoint==11){
            curCustomer.cowPointValue = @"大三公";
        }else{
        ...
        if ([PublicHttpTool shareInstance].cowPoint==customerPoint) {
           ...
           if ([PublicHttpTool shareInstance].cowPoint>customerPoint) {//庄家赢
               ...
               if (superValue>0) {
                   if (curCustomer.cashType==0||curCustomer.cashType==2) {
               ...
               curCustomer.isCow_customerWin = YES;
               CGFloat superValue = (1-[[fyjDict objectForKey:[NSString stringWithFormat:@"%d",customerPoint]]floatValue]/100)*[[fplDict objectForKey:[NSString stringWithFormat:@"%d",customerPoint]]floatValue]*[curCustomer.zhuangValue floatValue];
               
               CGFloat doubleValue = (1-[[fyjDict1 objectForKey:[NSString stringWithFormat:@"%d",customerPoint]]floatValue]/100)*[[fplDict1 objectForKey:[NSString stringWithFormat:@"%d",customerPoint]]floatValue]*[curCustomer.zhuangDuiValue floatValue];
               
               CGFloat normalValue = (1-[[fyjDict2 objectForKey:[NSString stringWithFormat:@"%d",customerPoint]]floatValue]/100)*[[fplDict2 objectForKey:[NSString stringWithFormat:@"%d",customerPoint]]floatValue]*[curCustomer.xianValue floatValue];
...
- (BOOL)fengzhuangCustomerInfo{
    ...
    [self.customerInfoList enumerateObjectsUsingBlock:^(CustomerInfo *curCustomer, NSUInteger idx, BOOL * _Nonnull stop) {
        if ([curCustomer.zhuangValue integerValue]==0&&[curCustomer.zhuangDuiValue integerValue]==0&&[curCustomer.xianValue integerValue]==0&&[curCustomer.washNumberValue integerValue]==0) {
